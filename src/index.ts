...
export class LinkChain {
  ...
  async run(): Promise<boolean> {
    try {
      ...
      console.log('[LinkChain] Score:', result.score.toFixed(4), '| Flagged:', result.flagged);
      if (result.flagged) {
        console.warn(\[LinkChain] ACTION REQUIRED: score \ exceeds threshold \\);
      }
      return true;
    } catch (err) {
      console.error('[LinkChain] Pipeline failed:', err);
      return false;
    }
  }
}
